...
public class MonoSingleton<T> : MonoBehaviour where T : Component
{
    ...
    {
        if (_instance == null)                                      // 先判断_instance是否已赋值
        {
            Type theType = typeof(T);

            _instance = (T)FindObjectOfType(theType);               // _instance没有,再确保场景中没有

            if (_instance == null)
            {
                var go = new GameObject(typeof(T).Name);
                _instance = go.AddComponent<T>();                   // 场景中没有,则创建一个空gameObject挂载T组件

                GameObject bootObj = GameObject.Find("BootObj");    //挂接到BootObj下
                if (bootObj == null)
                {
                    bootObj = new GameObject("BootObj");
                    DontDestroyOnLoad(bootObj);                     // BootObj是不会随着场景自动销毁的
                }
